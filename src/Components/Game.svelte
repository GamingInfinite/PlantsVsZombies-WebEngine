<script>
    ...
    for (let i = 0; i < lanes.length; i++) {
      ...
      if (getLaneJson(element).colors.length > 1) {
        for (let j = 0; j < 9; j++) {
          ...
          ctx.fillRect(
            boardXOffset + j * tileWidth,
            i * laneHeight + boardYOffset,
            tileWidth,
            laneHeight + 1
          );
        }
  ...
  function drawSeedPackets(ctx) {
    for (let i = 0; i < maxPlants; i++) {
      if (selectedSeed == setPicks[i]) {
        ...
        if (
          PlantSunCost[plantId] > sunCount ||
          rechargeTime[i][0] < rechargeTime[i][1]
        ) {
          ctx.fillStyle = "#000";
  ...
  function drawPacketAtCursor(ctx, i) {
    ...
    ctx.drawImage(
      loadedPorts[0],
      selectedX - packetWidth / 2,
      selectedY - packetHeight / 2,
      packetWidth,
      packetHeight
    );
  ...
  function drawPlants(ctx) {
    for (let i = 0; i < plantsToBeDrawn.length; i++) {
      ...
      let plantXOffset =
        tileWidth * 0.5 -
        (laneHeight * 0.9 * PlantSpriteSizeRatio[element.plant]) / 2;
      let plantYOffset = laneHeight * 0.06;

      //Add support for animations later !!IMPORTANT!!
      ctx.drawImage(
        firstFrame,
        boardXOffset + tileX * tileWidth + plantXOffset,
        tileY * laneHeight + boardYOffset + plantYOffset,
        laneHeight * 0.9 * PlantSpriteSizeRatio[element.plant],
        laneHeight * 0.9
      );
    }
  }
  ...
  function drawShovel(ctx, width) {
    ...
    if (!shovelSelect) {
      ...
      ctx.drawImage(
        extraIcons[0],
        selectedX - (width * 0.04) / 2,
        selectedY - (width * 0.04) / 2,
        width * 0.04,
        width * 0.04
      );
    }
  ...
  function shovelHitTest(x, y, width, height) {
    ...
    return (
      x >= shovelStartX &&
      x <= shovelStartX + width * 0.04 &&
      y >= shovelStartY &&
      y <= shovelStartY + width * 0.04
    );
  }
  ...
  function sunflowerCallback(tileX, tileY, id) {
    ...
    let plantXOffset =
      tileWidth * 0.5 -
      (laneHeight * 0.9 * PlantSpriteSizeRatio[plantHooks[id].type]) / 2;
    let plantYOffset = laneHeight * 0.06;
